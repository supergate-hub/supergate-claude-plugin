...
| dailies | `~/Documents/Obsidian Vault/notes/dailies/` |
...
2. **Daily Note 경로 확인**
```bash
DAILY_NOTE="$HOME/Documents/Obsidian Vault/notes/dailies/${TARGET_DATE}.md"
```
...
1. Bash로 해당 날짜에 수정된 .md 파일 찾기 (macOS 호환):
   find ~/Documents/"Obsidian Vault" -name "*.md" -type f -exec stat -f "%Sm %N" -t "%Y-%m-%d" {} \; 2>/dev/null | grep "{TARGET_DATE}" | awk '{print $2}' | grep -v "notes/dailies/"
...
## 경로
- 미팅 노트 패턴: ~/Documents/Obsidian Vault/notes/dailies/{TARGET_DATE}-*.md

## 실행 단계
1. Bash로 미팅 노트 파일 찾기:
   ls ~/Documents/"Obsidian Vault"/notes/dailies/{TARGET_DATE}-*.md 2>/dev/null
